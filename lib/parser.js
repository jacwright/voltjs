...
Parser.prototype.readLong = function() {
	...
	for (var i = 0; i < numBytes; i++) bytes.push(ctype.ruint8(this.buffer, endian, this.position + i));
	this.position += numBytes;
	return new BigInteger(bytes);
};
Parser.prototype.writeLong = function(value) {
	var bytes, numBytes = 8;
	if (typeof value === 'number') value = new BigInteger(value.toString());
	if ( !(value instanceof BigInteger)) throw new Error('Long type must be a BigInteger or Number');
	
	bytes = value.toByteArray();
	bytes = zeros(numBytes - bytes.length).concat(bytes);
	
	for (var i = 0; i < numBytes; i++) ctype.wsint8(bytes[i], endian, this.buffer, this.position + i);
	this.position += numBytes;
};
...
Parser.prototype.writeDate = function(value) {
	...
	else if (typeof value !== 'number') throw new Error('Date type must be a Date or number');
...
Parser.prototype.writeDecimal = function(value) {
	...
	if (typeof value != 'string' || !/^-?\d*\.?\d*$/.test(value)) throw new Error('Decimal type must be a numerical string or Number:' + value);
...
Parser.prototype.writeArray = function(type, value) {
	...
	if (match = type.match(arrExp)) {
		this.writeByte(TYPES_NUMBERS.array); // write type 'array' -99
		...
		this.writeByte(TYPES_NUMBERS[type]); // write type
		type == 'byte' ? this.writeInt(length) : this.writeShort(length); // write length
...
Parser.prototype.readVoltTable = function() {
	...
	for (var i = 0; i < columnCount; i++) {
		var type = TYPES_STRINGS[this.readByte()];
		columnTypes[i] = type;
...
Parser.prototype.readException = function(length) {
	if (length == 0) new Error('An exception has occurred');
	var ordinal = this.readByte();
	var theRest = this.readBinary(length - 1); // they don't have a spec for exceptions at this time, just skip it.
	if (ordinal == 1) return new Error('EEException');
	else if (ordinal == 2) return new Error('SQLException');
	else if (ordinal == 3) return new Error('ConstraintFailureException');
