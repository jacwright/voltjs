...
VoltJS.prototype.connect = function(username, password, host, port) {
...
VoltJS.prototype.login = function(username, password) {
	...
	this.socket.removeListener('data', this.onCall);
	this.socket.on('data', this.onLogin);
...
VoltJS.prototype.call = function(procedure) {
...
//		data = new Buffer(overflow.length + buffer.length),
//		length;
...
VoltJS.prototype.onLogin = function(buffer) {
	...
	this.socket.removeListener('data', this.onLogin);
	this.socket.on('data', this.onCall);
...
VoltJS.prototype.onCall = function(buffer) {
	...
	if (clientData in this._calls) {
		...
		else if (status != STATUS_CODES.SUCCESS) deferred.fail(new Error(statusString));
