...
function when(obj) {
	...
	var args = Array.prototype.slice.call(arguments),
		...
		createCallback = function(index) {
			return function(value) {
				...
				if (--count == 0) {
					if (failed) deferred.fail.apply(null, args);
					else deferred.fulfill.apply(null, args);
				}
				return value;
			};
		};
	...
	for (var i = 0, l = args.length; i < l; i++) {
		...
		if (obj && typeof obj.then === 'function') {
			obj.failed(failCallback).finished(createCallback(i));
		} else {
			--count;
		}
	}
